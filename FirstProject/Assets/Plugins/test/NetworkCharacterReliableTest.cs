...
public class NetworkCharacterReliableTest : MonoBehaviour {
	...
	void OnSerializeNetworkView(BitStream stream, NetworkMessageInfo info) {
		...
		if (stream.isWriting)
		{
			bool hasSwing = ControlSchemeInterface.instance.GetAxis(ControlAxis.ATTACK1) > 0f;
			bool swinging = false;
			if(animator){
				AnimatorStateInfo stateInfo = animator.GetCurrentAnimatorStateInfo(0);
				if(stateInfo.nameHash == slash1AnimationNameHash || stateInfo.nameHash == slash2AnimationNameHash){
					//Debug.Log("swinging: " + hasSwing);
					swinging = true;
					hasSwing = true;
				}
			}
			if(swinging){
				Debug.Log("swinging: " + swinging + " hasSwing: " + hasSwing);
			}
		...
		if (m_BufferedState[0].timestamp > interpolationTime)
		{
			...
			for (int i=0;i<m_TimestampCount;i++)
			{
				if (m_BufferedState[i].timestamp <= interpolationTime || i == m_TimestampCount-1)
				{
					...
					attacked = lhs.hasSwing || rhs.hasSwing;
					break;
				}
			}
		}
...
//				transform.rotation = latest.rot;// angularRotation * latest.rot;
	...
	void SynchAttack(bool attacked){
		if (animator && animator.enabled)
		{
			...
			if(stateInfo.nameHash == idleAnimationNameHash || stateInfo.nameHash == runAnimationNameHash){
				...
				if(attacked && stateInfo.normalizedTime > 0.6f){
					animator.SetBool("Slash", true);
					animator.SetInteger("SlashVariant", UnityEngine.Random.Range(1, 3));
				}
			}
